...
orderRouter.post("/create", userAuth, async (req, res) => {
  const { items, totalPrice, paymentMethod, shippingAddress, couponCode } =
    req.body;
    ...
    if (couponCode) {
      ...
      if (totalPrice < coupon.minimumOrderValue) {
        return res.status(400).json({
          message: `Minimum order value for coupon is ${coupon.minimumOrderValue}`,
        });
      }
      ...
      items.map(async (item) => {
        ...
        if (item.size) {
          const sizeOption = product.sizes.find((s) =>
            s.size.equals(item.size)
          );
          if (!sizeOption || sizeOption.stock < item.quantity) {
            throw new Error(
              `Insufficient stock for size selection of product ${product.name}`
            );
          }
          // Update stock
        ...
        if (item.color) {
          const colorOption = product.colors.find((c) =>
            c.color.equals(item.color)
          );
          if (!colorOption || colorOption.stock < item.quantity) {
            throw new Error(
              `Insufficient stock for color selection of product ${product.name}`
            );
          }
          // Update stock
...
orderRouter.get("/admin", adminAuth, async (req, res) => {
  try {
    const orders = await Order.find()
      .populate("user", "name email") // Populate user with name and email
      .populate("items.product", "name price") // Populate product with name and price
      .populate("items.color", "name") // Populate color name
      .populate("items.size", "name") // Populate size name
      .populate("coupon", "code discount") // Populate coupon details
