...
productRouter.get("/admin", async (req, res) => {
  try {
    ...
    // Fetch paginated products and populate their associated subcategory
...
productRouter.put("/:productId", userAuth, async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      updates.sellingPrice > updates.price
    ) {
      return res.status(400).json({
  ...
  try {
    ...
    const query = {
      ...
      $or: [
        {
          categories: {
            $in: Array.isArray(product.categories)
              ? product.categories
              : [product.categories],
          },
        },
        {
          subcategories: {
            $in: Array.isArray(product.subcategories)
    ...
    res
      .status(200)
      .json({ success: true, similarProducts: updatedSimilarProducts });
  } catch (error) {
